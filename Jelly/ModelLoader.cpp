...
QSharedPointer<MeshModel> ModelLoader::processMesh(aiMesh *mesh)
{
	QSharedPointer<MeshModel> newMesh(new MeshModel);
	...
	for (uint t = 0; t<mesh->mNumFaces; ++t)
	{
		...
		{
			qDebug() << "Warning: MeshModel face with not exactly 3 indices, ignoring this primitive." << face->mNumIndices;
			continue;
		}
...
void ModelLoader::processNode(const aiScene *scene, aiNode *node, Node *parentNode, Node &newNode)
{
	...
	{
		QSharedPointer<MeshModel> mesh = m_meshes[node->mMeshes[imesh]];
